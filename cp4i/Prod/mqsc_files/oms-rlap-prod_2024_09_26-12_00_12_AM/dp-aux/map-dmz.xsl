...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template mode="request" match="operation[@dmz='true' 
      ...
      <xsl:variable name="config">
          <xsl:choose>
              <xsl:when test="function-available('dpgui:get-config')">
                  <xsl:copy-of select="dpgui:get-config('default', 'WebGUI', '', 0)/configuration"/>
              </xsl:when>
          </xsl:choose>
      </xsl:variable>
      ...
      <!-- output -->
      <operation type='{@type}'>
          <xsl:copy-of select="$config/configuration"/>
      </operation>
  </xsl:template>
  ...
  <xsl:template mode="request" match="operation[@dmz='true' 
    ...
    <xsl:variable name="config">
      <xsl:choose>
        <xsl:when test="function-available('dpgui:get-config')">
          <xsl:copy-of select="dpgui:get-status('default', 'EthernetInterfaceStatus', '0', '0', '')/statistics"/>
        </xsl:when>
      </xsl:choose>
    </xsl:variable>
    ...
    <xsl:variable name="status">
      <xsl:choose>
        <xsl:when test="function-available('dpgui:get-status')">
          <xsl:copy-of select="dpgui:get-status('default', request-class, '0', '0', '')/statistics"/>
        </xsl:when>
      </xsl:choose>
    </xsl:variable>
