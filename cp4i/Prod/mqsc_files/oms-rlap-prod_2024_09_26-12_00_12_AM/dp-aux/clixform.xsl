...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="CONFIG-ID" select="'(undefined)'" />
    ...
    <!-- fetches the passed in value for an action parameter or the single defined
         default value for that parameter in the mgmt-schema, if defined -->
    <func:function name="dpfunc:getParameterValue">
        <xsl:param name="args" />
        ...
        <xsl:variable name="val">
          <xsl:choose>
              <xsl:when test="$args/*[local-name() = $name]">
                  <xsl:value-of select="$args/*[local-name() = $name]"/>
              </xsl:when>
              <xsl:otherwise>
                  <xsl:value-of select="$pNode//parameter[@name=$name]/default"/>
              </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
   ...
   <!-- default to false for use by dpe:transform calls to clixform
    ...
    <xsl:template name="version-comment">
        ...
        <xsl:if test="(@build and @timestamp)">
            ...
            <xsl:value-of select="normalize-space($CONFIG-ID)"/>
    ...
    <xsl:template mode="XMLManager" match="CacheMemorySize">
        <xsl:if test="text()!=''">
            <xsl:value-of select="concat('xsl cache memorysize ', dpfunc:quoesc(../@name), ' ', dpfunc:quoesc(text()), $eol)"/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template mode="XMLManager" match="SupportTxWarn">
        <xsl:variable name="itx-is-licensed">
            <xsl:call-template name="is-licensed">
                <xsl:with-param name="featureToCheck" select="'ITX'"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:if test="($itx-is-licensed = 'true')">
  ...
  <!-- ************************************************************ -->
    ...
    <xsl:template mode="cli-object" match="LogTarget">
        <xsl:choose>
            ...
            <xsl:when test="(@name='default-log') or (@name='default-log-console')">
                <xsl:if test="($delta-config=true())">
                    <xsl:value-of select="concat($eol, 'no logging object ', @name,' *', $eol )"/>
                    <xsl:value-of select="concat($eol, 'no logging event ', @name,' *', $eol )"/>
                    <xsl:value-of select="concat($eol, 'no logging eventcode ', @name,' *', $eol )"/>
                    <xsl:value-of select="concat($eol, 'no logging eventfilter ', @name,' *', $eol )"/>
                    <xsl:value-of select="concat($eol, 'no logging ipfilter ', @name,' *', $eol )"/>
                    <xsl:value-of select="concat($eol, 'no logging trigger ', @name,' *', $eol )"/>
                </xsl:if>
    ...
    <xsl:template mode="DefaultLogTarget" match="LogObjects">
      <xsl:param name="objName"/>
        <xsl:value-of select="concat( 'logging object ', $objName,
                                      ' ', dpfunc:quoesc(Class),
    ...
    <xsl:template mode="DefaultLogTarget" match="LogEvents[not(Class = 'webgui')]">
      <xsl:param name="objName"/>
        <xsl:value-of select="concat( 'logging event ', $objName, ' ', dpfunc:quoesc(Class), ' ', dpfunc:quoesc(Priority), $eol )"/>
    </xsl:template>

    <xsl:template mode="DefaultLogTarget" match="LogEventCode">
      <xsl:param name="objName"/>
        <xsl:value-of select="concat( 'logging eventcode ', $objName, ' ', dpfunc:quoesc(text()), $eol )"/>
    </xsl:template>

    <xsl:template mode="DefaultLogTarget" match="LogEventFilter">
      <xsl:param name="objName"/>
        <xsl:value-of select="concat( 'logging eventfilter ', $objName, ' ', dpfunc:quoesc(text()), $eol )"/>
    </xsl:template>

    <xsl:template mode="DefaultLogTarget" match="LogIPFilter">
      <xsl:param name="objName"/>
        <xsl:value-of select="concat( 'logging ipfilter ', $objName, ' ', dpfunc:quoesc(IPAddress), $eol )"/>
    </xsl:template>

    <xsl:template mode="DefaultLogTarget" match="LogTriggers">
      <xsl:param name="objName"/>
        <xsl:value-of select="concat('logging trigger ', $objName, 
                                     ' ', dpfunc:quoesc(ID),
    ...
    <xsl:template mode="cli-object" match="SSHService">
        ...
        <xsl:choose>
          <xsl:when test="(mAdminState='enabled')">
            <xsl:choose>
              <xsl:when test="(ConnectionLimit!=0)">
                <xsl:value-of select="concat($eol, 'ssh ', dpfunc:quoesc(LocalAddress), ' ', dpfunc:quoescne(LocalPort), ' ', dpfunc:quoescne(ConnectionLimit), $eol)"/>
              </xsl:when>
              <xsl:otherwise>
   ...
   <xsl:template mode="HTTPUserAgent" match="*">
    ...
    <xsl:template mode="cli-object" match="APIGateway">
        <xsl:value-of select="concat($eol, 'apigw ', dpfunc:quoesc(@name), $eol)"/>
        <xsl:if test="($delta-config=true())">
            <xsl:value-of select="concat('  reset', $eol)"/>
        </xsl:if>
        <xsl:apply-templates mode="APIGateway"/>
        <xsl:value-of select="concat('exit', $eol)"/>
    </xsl:template>

    <xsl:template mode="APIGateway" match="ProxyPolicies">
        <xsl:value-of select="concat('  proxy ', dpfunc:quoesc(RegExp))"/>
        <xsl:choose>
            <xsl:when test="(Skip='off')">
                <xsl:value-of select="concat(' ', dpfunc:quoesc(RemoteAddress), ' ', dpfunc:quoescne(RemotePort), ' ', dpfunc:quoescne(UserName), ' ', dpfunc:quoescne(Password))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="concat(' none')"/>
            </xsl:otherwise>
     ...
     <xsl:template mode="APIGateway" match="*">
        <xsl:apply-templates mode="DefaultProperty" select=".">
            <xsl:with-param name="objName" select="name(..)"/>
            <xsl:with-param name="pName" select="name()"/>
            <xsl:with-param name="pValue" select="."/>
    ...
    <xsl:template mode="cli-object" match="AnalyticsEndpoint">
        <xsl:value-of select="concat($eol, 'analytics-endpoint ', dpfunc:quoesc(@name), $eol)"/>
        <xsl:if test="($delta-config=true())">
            <xsl:value-of select="concat('  reset', $eol)"/>
        </xsl:if>
        <xsl:apply-templates mode="AnalyticsEndpoint"/>
        <xsl:value-of select="concat('exit', $eol)"/>
    </xsl:template>

    <xsl:template mode="AnalyticsEndpoint" match="ProxyPolicies">
        <xsl:value-of select="concat('  proxy ', dpfunc:quoesc(RegExp))"/>
        <xsl:choose>
            <xsl:when test="(Skip='off')">
                <xsl:value-of select="concat(' ', dpfunc:quoesc(RemoteAddress), ' ', dpfunc:quoescne(RemotePort), ' ', dpfunc:quoescne(UserName), ' ', dpfunc:quoescne(Password))"/>
     ...
     <xsl:template mode="AnalyticsEndpoint" match="*">
        <xsl:apply-templates mode="DefaultProperty" select=".">
            <xsl:with-param name="objName" select="name(..)"/>
            <xsl:with-param name="pName" select="name()"/>
            <xsl:with-param name="pValue" select="."/>
    ...
    <xsl:template mode="CanonicalProperty" match="*">
        ...
        <xsl:if test="not($propertyElement/@status='obsolete')">
    ...
    <xsl:template name="is-licensed">
        ...
        <!-- If that still did not work; for docker try /opt/ibm/datapower/drouter/webgui/Licenses.xml -->
        <xsl:variable name="features">
           <xsl:choose>
              <xsl:when test="not($features2) or $features2='' ">
                 <xsl:copy-of select="document('/opt/ibm/datapower/drouter/webgui/Licenses.xml')"/>
              </xsl:when>
              <xsl:otherwise>
                 <xsl:copy-of select="$features2"/>
              </xsl:otherwise>
           </xsl:choose>
        </xsl:variable>
  ...
  <xsl:template match="args[action='RefreshAPIGWDocument']" mode="cli-actions">
      <!-- Turn url supplied via RefreshAPIDocument into a pattern for clear command. -->
      <xsl:variable name="escapedURL" select="regexp:replace(Document,'([?*\]\[.^$])','g','\$1')"/>
      <xsl:value-of select="concat(
                            'configure terminal', $eol,
                            'flush-apigw-document ', dpfunc:quoesc(APIGateway), ' ',  dpfunc:quoesc($escapedURL), $eol)"/>
  </xsl:template>
  ...
  <xsl:template match="args[action='APIOAuthCacheDelete']" mode="cli-actions">
      <xsl:value-of select="concat(
                            'configure terminal', $eol,
                            'crypto', $eol,
                            'api-oauth-cache-delete ',
                            dpfunc:quoesc(Provider), ' ',
                            dpfunc:quoesc(CacheType), $eol)"/>
  </xsl:template>
...
<xsl:template match="args[action='BootDelete']" mode="cli-actions">
  ...
  <xsl:template match="args[action='PurgeB2BpGateway']" mode="cli-actions">
      <xsl:variable name="pNode" select="$schema/action-objects/action[@name='PurgeB2BpGateway']"/>

      <xsl:value-of select="concat('configure terminal', $eol,
                                   'purge-b2bp-gateway ', dpfunc:quoesc(B2BGatewayName), ' ',
                                   dpfunc:quoesc(dpfunc:getParameterValue(.,$pNode,'PurgeIncomplete')), ' ',
                                   dpfunc:quoesc(dpfunc:getParameterValue(.,$pNode,'PurgeUnexpiredEBMS')), $eol)"/>
  </xsl:template>

  <xsl:template match="args[action='PurgeB2BpAll']" mode="cli-actions">
      <xsl:variable name="pNode" select="$schema/action-objects/action[@name='PurgeB2BpAll']"/>
  ...
  <xsl:template match="args[action='TraceRoute']" mode="cli-actions">
      <xsl:variable name="traceRouteCmd">
          ...
          <xsl:if test="string(LocalAddress) != '' and string(LocalAddress) != '0'">
              <xsl:value-of select="concat(' ', dpfunc:quoesc(string(LocalAddress)))"/>
          </xsl:if>
      </xsl:variable>
      <xsl:value-of select="concat('traceroute', $traceRouteCmd, $eol)" />
  </xsl:template>
  ...
  <xsl:template match="args[action='SecureBackup']" mode="cli-actions">
      <xsl:variable name="idg-is-licensed">
          <xsl:call-template name="is-licensed">
              <xsl:with-param name="featureToCheck" select="'IDG'"/>
          </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="secureBackupCmd">
          <xsl:value-of select="concat(' ', dpfunc:quoesc(string(cert)))" />
          <xsl:value-of select="concat(' ', dpfunc:quoesc(string(destination)))" />
          <xsl:if test="($idg-is-licensed = 'true')">
              <!-- Additional parameters for IDG but not for IMQA -->
              <xsl:if test="string(include-iscsi) != '' or string(include-raid) != ''">
                  <xsl:value-of select="concat(' ', dpfunc:quoesc(string(include-iscsi)))" />
              </xsl:if>
              <xsl:if test="string(include-raid) != ''">
                  <xsl:value-of select="concat(' ', dpfunc:quoesc(string(include-raid)))" />
              </xsl:if>
          </xsl:if>
      </xsl:variable>
      <xsl:value-of select="concat('configure terminal', $eol,
                                   'secure-backup', $secureBackupCmd, $eol)" />
  </xsl:template>
  
  <xsl:template match="args[action='SecureRestore']" mode="cli-actions">
      <xsl:variable name="idg-is-licensed">
          <xsl:call-template name="is-licensed">
              <xsl:with-param name="featureToCheck" select="'IDG'"/>
          </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="secureRestoreCmd">
          <xsl:value-of select="concat(' ', dpfunc:quoesc(string(cred)))" />
          <xsl:value-of select="concat(' ', dpfunc:quoesc(string(source)))" />
          <xsl:if test="string(validate) != ''">
              <xsl:value-of select="concat(' ', dpfunc:quoesc(string(validate)))" />
          </xsl:if>
          <xsl:if test="($idg-is-licensed = 'true')">
              <!-- Additional parameter for IDG but not for IMQA -->
              <xsl:if test="string(BackupMachineType) != ''">
                  <xsl:if test="string(validate) = ''">
                      <xsl:value-of select="concat(' ', 'off')" />
                  </xsl:if>
                  <xsl:value-of select="concat(' ', dpfunc:quoesc(string(BackupMachineType)))" />
              </xsl:if>
          </xsl:if>
      </xsl:variable>
      <xsl:value-of select="concat('configure terminal', $eol,
                                   'secure-restore', $secureRestoreCmd, $eol)" />
  </xsl:template>
  ...
  <xsl:template match="args[action='SetRBMDebugLog']" mode="cli-actions">
      ...
      <xsl:variable name="log-config-console">
        <xsl:call-template name="do-mgmt-request">
            <xsl:with-param name="session">
                <xsl:choose>
                    <xsl:when test="../args/session"><xsl:value-of select="../args/session"/></xsl:when>
            ...
            <xsl:with-param name="request">
                <request>
                  <operation type="get-config">
                    <request-class>LogTarget</request-class>
                    <request-name>default-log-console</request-name>
                  </operation>
                </request>
            </xsl:with-param>
        </xsl:call-template>
      ...
      <xsl:choose>
          <xsl:when test="string(RBMLog) = 'on'">
              ...
              <xsl:if test="$log-config-console/response/operation[@type='get-config']
                            /configuration/*[(local-name()='LogTarget') and (@name='default-log-console')]">
                  <xsl:value-of select="concat('logging event default-log-console rbm debug', $eol)"/>
              </xsl:if>
          </xsl:when>
          ...
          <xsl:otherwise>
              ...
              <xsl:if test="$log-config-console/response/operation[@type='get-config']
                            /configuration/*[(local-name()='LogTarget') and (@name='default-log-console')]
                            /LogEvents[Class='rbm']">
                  <xsl:value-of select="concat('no logging event default-log-console rbm', $eol)"/>
              </xsl:if>
          </xsl:otherwise>
      </xsl:choose>
  </xsl:template>
      ...
      <xsl:if test="InternalLog">
          <xsl:choose>
              <xsl:when test="string(InternalLog) = 'on'">
                  ...
                  <xsl:if test="$log-config-console/response/operation[@type='get-config']
                                  /configuration/*[(local-name()='LogTarget') and (@name='default-log-console')]">
                      <xsl:value-of select="concat('logging event default-log-console webgui ', dpfunc:quoesc(LogLevel), $eol)"/>
                  </xsl:if>
              </xsl:when>
              ...
              <xsl:if test="$log-config-console/response/operation[@type='get-config']
                              /configuration/*[(local-name()='LogTarget') and (@name='default-log-console')]
                              /LogEvents[Class='webgui']">
                  <xsl:value-of select="concat('no logging event default-log-console webgui', $eol)"/>
              </xsl:if>
          </xsl:otherwise>
      </xsl:choose>
      </xsl:if>
  ...
  <xsl:template match="text()" mode="HTTPUserAgent"/>
  <xsl:template match="text()" mode="APIGateway"/>
